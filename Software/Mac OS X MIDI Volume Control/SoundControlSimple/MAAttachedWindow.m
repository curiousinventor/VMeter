// much of this code is from http://mattgemmell.com/2007/10/03/maattachedwindow-nswindow-subclass
...
- (void)_updateGeometry
{
	...
	if (contentRect.origin.x - _distance < screenRect.origin.x) {
		contentRect.origin.x = screenRect.origin.x + _distance;
	}
	if (contentRect.origin.x + contentRect.size.width + _distance > screenRect.origin.x + screenRect.size.width) {
		contentRect.origin.x = (screenRect.origin.x + screenRect.size.width) - (contentRect.size.width + _distance);
	}
...
- (NSBezierPath *)_backgroundPath
{
	...
	float arrowX = (_point.x - minX) - [self frame].origin.x;
    ...
    if (_side == MAPositionBottomRight) {
		...
		[path lineToPoint:NSMakePoint(arrowX - halfArrowWidth, maxY)];
        // Draw arrow.
        [self _appendArrowToPath:path];
