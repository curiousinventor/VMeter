...
void removal_callback(void *context, 
                                     IOReturn result, 
                                     void *sender, 
...
hid_device * HID_API_EXPORT hid_open(unsigned short vendor_id, unsigned short product_id, wchar_t *serial_number)
{
    ...
    // register call back function for when device is removed?
    IOHIDManagerRegisterDeviceRemovalCallback(hid_mgr, 
                                              removal_callback, 
                                              handle) ;  
...
void removal_callback (void *context, IOReturn result, void *sender,IOHIDDeviceRef device) {
  ...
  //  hid_close(context);
    ...
    dev->alive = 0;
	//free(dev); // move to the hid read function?  If we free here, then we lose access to dev->alive
...
// this should return -5 if VMeter is unplugged, but it's actually crashing a lot...
int HID_API_EXPORT hid_read(hid_device *dev, unsigned char *data, size_t length)
{
	int ret_val = -1;
 
    if (dev->alive == 0) {
        free(dev); // used to do this in the removal_callback, but that was killing access to the dev->alive variable.
        return -5;
            
    }
    
//    if (dev->device_handle == nil) {
//        dev->alive = 0;
//        return -5;
//    }
